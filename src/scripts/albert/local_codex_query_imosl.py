...
if __name__ == "__main__":
    ...
    parser.add_argument("--aligned_path", type=str,
                        default="data/imosl_autoformalization_isabelle")
    parser.add_argument("--dump_path", type=str,
                        default="dump/imosl")
    parser.add_argument("--log_path", type=str, default="dump/imosl/log")
    ...
    parser.add_argument("--progress_path", type=str, default="dump/imosl/progress")
    ...
    name_to_info = {}
    if os.path.isdir(algined_path):
        problem_list = os.listdir(algined_path)
        for problem_file in problem_list:
            with open(os.path.join(algined_path, problem_file)) as f:
                line = json.load(f)
            problem_name = problem_file[:-5] #remove .json
        ...
        with open(algined_path) as f:
            ...
            for problem_name, line in metadata.items():
                ...
                formal_statement = line["isa_formal_state"]
    ...
    python_file = os.path.join(ROOTDIR, "autoformalization/query_and_store.py")
    for sub_lists in tqdm(all_sub_lists):
        a_list_of_jobs(sub_lists, progress_path, python_file_to_execute=python_file)

    # executor.map_array(a_list_of_jobs, all_sub_lists, [progress_path] * len(all_sub_lists))
